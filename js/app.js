(function () {
  ...
  function render() {
    ...
    if (systems) systems.innerHTML = (site.systems || []).filter((item) => item.published !== false).map((item) => `<article class="system-card"><span class="eyebrow">${escape(item.icon || "FSRP System")}</span><h3>${escape(item.title)}</h3><p>${escape(item.description || item.body)}</p></article>`).join("");
    const steps = document.getElementById("join-steps");
    if (steps) steps.innerHTML = (site.joinSteps || []).filter((item) => item.published !== false).map((item, index) => `<article class="value-item"><b>${escape(item.number || `0${index + 1}`)}</b><div><h3>${escape(item.title)}</h3><p>${escape(item.description || item.body)}</p></div></article>`).join("");
    const faq = document.getElementById("faq-list");
    if (faq) faq.innerHTML = (site.faqs || []).filter((item) => item.published !== false).map((item, index) => `<article class="faq-item"><button class="faq-question" data-faq="${index}">${escape(item.question)}<span>+</span></button><div class="faq-answer" hidden>${escape(item.answer)}</div></article>`).join("");
    const support = document.getElementById("support-grid");
    if (support) support.innerHTML = (site.support || []).filter((item) => item.published !== false).map((item) => `<article class="support-card"><span class="eyebrow">${escape(item.icon || "Support")}</span><h3>${escape(item.title)}</h3><p>${escape(item.description || item.body)}</p><a class="btn btn-secondary btn-small" href="${escape(item.url || site.links.discord)}" target="_blank" rel="noopener">${escape(item.label || "Open Support")}</a></article>`).join("");

    renderAnnouncements(site);
    renderEvents(site);
    renderTimeline(site);
    renderGallery(site);
    renderMarketplace(site);
    renderRules(site);
    renderScene(site);
  ...
  function renderDepartments(root, site, filter) {
    ...
    root.innerHTML = (site.departments || [])
      .filter((department) => department.published !== false && (filter === "all" || department.category === filter))
      .map((department) => `<article class="dept-card" style="--dept-color:${escape(department.color)};${department.image ? `background-image:linear-gradient(160deg,rgba(5,10,17,.70),rgba(5,10,17,.97)),url('${escape(department.image)}');background-size:cover;background-position:center;` : ""}"><div class="dept-code">${escape(department.short || department.code)}</div><span class="badge ${department.status === "Open" ? "is-live" : ""}">${escape(department.status)}</span><h3>${escape(department.name)}</h3><p>${escape(department.description)}</p><a class="btn btn-secondary btn-small" href="${escape(department.link || site.links.discord)}" target="_blank" rel="noopener">View in Discord</a></article>`)
      .join("");
  }

  function renderAnnouncements(site) {
    const root = document.getElementById("announcement-grid");
    if (root) root.innerHTML = (site.announcements || []).filter((item) => item.published !== false).map((item) => `<article class="bento-card span-4"><span class="eyebrow">${escape(item.category || "Announcement")}</span><h3>${escape(item.title)}</h3><p>${escape(item.body)}</p><small>${escape(item.date || "")}</small></article>`).join("");
    const featured = (site.announcements || []).filter((item) => item.published !== false).find((item) => item.featured) || (site.announcements || []).find((item) => item.published !== false);
    if (featured) {
  ...
  function renderEvents(site) {
    ...
    if (root) root.innerHTML = (site.events || []).filter((item) => item.published !== false).map((item) => `<article class="event-card"><span class="eyebrow">Event</span><h3>${escape(item.title)}</h3><p>${escape(item.description)}</p><strong>${escape(item.date)}</strong></article>`).join("") || '<p class="muted">No event is currently scheduled.</p>';
    const next = (site.events || []).find((item) => item.published !== false);
  ...
  function renderTimeline(site) {
    ...
    if (root) root.innerHTML = (site.timeline || []).filter((item) => item.published !== false).map((item) => `<article class="timeline-item"><span class="eyebrow">${escape(item.date)}</span><h3>${escape(item.title)}</h3><p>${escape(item.description)}</p></article>`).join("");
  }

  function renderGallery(site) {
    const root = document.getElementById("media-gallery");
    if (!root) return;
    root.innerHTML = (site.gallery || []).filter((item) => item.published !== false).map((item) => `<article class="media-item">${item.type === "video" ? `<video controls preload="metadata" src="${escape(item.url)}"></video>` : `<img loading="lazy" src="${escape(item.url)}" alt="${escape(item.title)}">`}<h3>${escape(item.title)}</h3></article>`).join("") || '<p class="muted">No public media has been published.</p>';
  }

  function renderMarketplace(site) {
    const root = document.getElementById("market-grid");
    if (!root) return;
    const items = (site.marketplace || []).filter((item) => item.published !== false);
    root.innerHTML = items.map((item) => `<article class="market-card ${item.featured ? "featured" : ""}"><span class="badge ${item.featured ? "is-warning" : ""}">${escape(item.tag || "Official")}</span><h3>${escape(item.name || item.title)}</h3><p>${escape(item.description)}</p>${(item.benefits || []).length ? `<ul>${item.benefits.map((benefit) => `<li>${escape(benefit)}</li>`).join("")}</ul>` : ""}<a class="btn btn-secondary btn-small" href="${escape(item.buttonUrl || item.url || site.links.discord)}" target="_blank" rel="noopener">${escape(item.buttonLabel || "View in Discord")}</a></article>`).join("") || '<p class="muted">Marketplace information has not been published.</p>';
  }
  ...
  function renderRules(site) {
    ...
    navigation.innerHTML = (site.rules || []).filter((item) => item.published !== false).map((item, index) => `<button class="${index === 0 ? "is-active" : ""}" data-rule="${index}">${escape(item.title)}</button>`).join("");
    const show = (index) => {
      const rule = (site.rules || []).filter((item) => item.published !== false)[index];
      if (!rule) return;
  ...
  function renderMaintenance(site) {
    ...
    const maintenanceEnabled = site.maintenance?.enabled === true
      ...
      && Number(site.maintenance?.safetyVersion) >= 3;
